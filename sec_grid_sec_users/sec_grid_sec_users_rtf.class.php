...
$_SESSION['scriptcase']['sec_grid_sec_users']['contr_erro'] = 'off'; 
   ...
   {
             $conteudo_x =  $this->data_senha;
             nm_conv_limpa_dado($conteudo_x, "YYYY-MM-DD");
             if (is_numeric($conteudo_x) && strlen($conteudo_x) > 0) 
             { 
                 $this->nm_data->SetaData($this->data_senha, "YYYY-MM-DD  ");
                 $this->data_senha = $this->nm_data->FormataSaida($this->nm_data->FormatRegion("DT", "ddmmaaaa"));
             } 
         $this->data_senha = NM_charset_to_utf8($this->data_senha);
         $this->data_senha = str_replace('<', '&lt;', $this->data_senha);
         $this->data_senha = str_replace('>', '&gt;', $this->data_senha);
         $this->Texto_tag .= "<td>" . $this->data_senha . "</td>\r\n";
   }
   //----- data_expirar_senha
   function NM_export_data_expirar_senha()
   {
             $conteudo_x =  $this->data_expirar_senha;
             nm_conv_limpa_dado($conteudo_x, "YYYY-MM-DD");
             if (is_numeric($conteudo_x) && strlen($conteudo_x) > 0) 
             { 
                 $this->nm_data->SetaData($this->data_expirar_senha, "YYYY-MM-DD  ");
                 $this->data_expirar_senha = $this->nm_data->FormataSaida($this->nm_data->FormatRegion("DT", "ddmmaaaa"));
             } 
         $this->data_expirar_senha = NM_charset_to_utf8($this->data_expirar_senha);
         $this->data_expirar_senha = str_replace('<', '&lt;', $this->data_expirar_senha);
         $this->data_expirar_senha = str_replace('>', '&gt;', $this->data_expirar_senha);
         $this->Texto_tag .= "<td>" . $this->data_expirar_senha . "</td>\r\n";
   }
