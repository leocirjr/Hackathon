...
$_SESSION['scriptcase']['sec_grid_sec_users']['contr_erro'] = 'off'; 
      ...
      while (!$rs->EOF)
      {
         ...
         $this->data_senha = $rs->fields[4] ;  
         $this->data_expirar_senha = $rs->fields[5] ;  
   ...
   { 
      ...
      { 
          ...
          if ($Cada_col == "data_senha" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $this->count_span++;
              $current_cell_ref = $this->calc_cell($this->Xls_col);
              $SC_Label = NM_charset_to_utf8($SC_Label);
          ...
          if ($Cada_col == "data_expirar_senha" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $this->count_span++;
              $current_cell_ref = $this->calc_cell($this->Xls_col);
              $SC_Label = NM_charset_to_utf8($SC_Label);
   ...
   function NM_export_data_senha()
   {
         ...
         $this->NM_ctrl_style[$current_cell_ref]['end'] = $this->Xls_row;
         $this->data_senha = substr($this->data_senha, 0, 10);
         if (empty($this->data_senha) || $this->data_senha == "0000-00-00")
         {
             if ($this->Use_phpspreadsheet) {
                 $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->data_senha, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
             }
             else {
                 $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->data_senha, PHPExcel_Cell_DataType::TYPE_STRING);
             }
         }
         else
         {
             $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->data_senha);
             $this->NM_ctrl_style[$current_cell_ref]['format'] = $this->SC_date_conf_region;
         }
         $this->Xls_col++;
   }
   //----- data_expirar_senha
   function NM_export_data_expirar_senha()
   {
         $current_cell_ref = $this->calc_cell($this->Xls_col);
         if (!isset($this->NM_ctrl_style[$current_cell_ref])) {
             $this->NM_ctrl_style[$current_cell_ref]['ini'] = $this->Xls_row;
         ...
         $this->NM_ctrl_style[$current_cell_ref]['end'] = $this->Xls_row;
         $this->data_expirar_senha = substr($this->data_expirar_senha, 0, 10);
         if (empty($this->data_expirar_senha) || $this->data_expirar_senha == "0000-00-00")
         {
             if ($this->Use_phpspreadsheet) {
                 $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->data_expirar_senha, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
             }
             else {
                 $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->data_expirar_senha, PHPExcel_Cell_DataType::TYPE_STRING);
             }
         }
         else
         {
             $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->data_expirar_senha);
             $this->NM_ctrl_style[$current_cell_ref]['format'] = $this->SC_date_conf_region;
         }
         $this->Xls_col++;
   }
   ...
   //----- data_senha
   function NM_sub_cons_data_senha()
   {
         $this->data_senha = substr($this->data_senha, 0, 10);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->data_senha;
   ...
   //----- data_expirar_senha
   function NM_sub_cons_data_expirar_senha()
   {
         $this->data_expirar_senha = substr($this->data_expirar_senha, 0, 10);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->data_expirar_senha;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "data";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "center";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = $this->SC_date_conf_region;
         $this->Xls_col++;
